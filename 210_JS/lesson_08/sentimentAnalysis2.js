...
function sentiment(text) {
  ...
  var positives = wordList.filter(function (word) {
    return !!(word.match(POSITIVE_REGEX));
  });
  var negatives = wordList.filter(function (word) {
    return !!(word.match(NEGATIVE_REGEX));
  });
  var textSentiment;
...
var POSITIVE_REGEX = /(fortunes?)|(dream(s|t|ed)?)|(love(s|d)?)|(respect(s|ed)?)|(patien(ce|t)?)|(devout(ly)?)|(nobler?)|(resolut(e|ion)?)/gi;
var NEGATIVE_REGEX = /(die(s|d)?)|(heartached?)|(death)|(despise(s|d)?)|(scorn(s|ed)?)|(weary)|(troubles?)|(oppress(es|ed|or('s)?)?)/gi;
